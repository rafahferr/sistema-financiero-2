...
async function recalcularDivida(dividaId: number) {
  ...
  const status: Divida['status'] =
    valorPago >= divida.valorTotal
      ? 'quitada'
      : divida.dataVencimento && divida.dataVencimento < hoje
        ? 'atrasada'
        : 'em_aberto';

  await db.dividas.update(dividaId, { valorPago, status });
}
...
/**
 * Converte em dívida toda despesa não paga de um mês já encerrado (mês/ano anterior ao atual).
 * O lançamento original é removido de Fluxos — a dívida passa a ser o único registro do valor pendente.
 * Lançamentos já vinculados a uma dívida (`pagamentoDividaId`) são ignorados, pois já são tratados como
 * pagamento de dívida existente, não como uma pendência nova.
 *
  ...
  for (const l of candidatos) {
    ...
    await db.transaction('rw', db.dividas, db.lancamentos, async () => {
      ...
      if (existente?.id) {
        ...
        await db.dividas.update(existente.id, {
          valorTotal: existente.valorTotal + l.valor,
          dataVencimento: vencimentoMaisRecente,
        });
        dividasAfetadas.add(existente.id);
      } else {
        const novoId = await db.dividas.add({
        ...
        dividasAfetadas.add(novoId as number);
      }

      await db.lancamentos.delete(l.id!);
    });
    total += l.valor;
  }
...
/** Roda a limpeza de duplicatas e a conversão automática uma única vez por carregamento do app. */
export function useConversaoAutomaticaDividas() {
  const [resultado, setResultado] = useState<{ convertidos: number; total: number; mesclados: number } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (convertidos > 0 || mesclados > 0) {
        setResultado({ convertidos, total, mesclados });
      }
    })();
  }, []);
